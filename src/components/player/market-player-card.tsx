...
export function MarketPlayerCard({
  ...
  const {
    ...
    percentDiff: pricePercentDiff,
    isGoodDeal,
  } = calculatePriceDifference(
    player.salePrice,
    player.playerMaster.marketValue
  )

  return (
    <Card
      className={`hover:shadow-md transition-shadow ${isGoodDeal ? 'border-green-200' : ''}`}
    >
      ...
      <CardContent className="space-y-3">
        <div className="flex items-center justify-between">
          ...
          <span className="font-medium text-slate-800">
            {player.playerMaster.team?.name || 'Unknown'}
          </span>
        </div>
        ...
        <div className="grid grid-cols-2 gap-4">
          ...
          <div>
            ...
            <div
              className={`font-medium ${isGoodDeal ? 'text-green-600' : 'text-red-600'}`}
            >
              {salePriceFormatted}
            </div>
          </div>
        ...
        {priceDifference !== 0 && (
          ...
          >
            ...
            {formatCurrency(Math.abs(priceDifference))} ({pricePercentDiff}%)
          </div>
        )}
        ...
        <div className="flex items-center justify-between">
          ...
          <span className="font-medium text-slate-800">
            {player.playerMaster.points} (
            {player.playerMaster.averagePoints.toFixed(1)} avg)
          </span>
        </div>
          ...
          <div className="border-t pt-3">
            {biddingPlayer === player.id ? (
              <div className="space-y-3">
                <div className="flex gap-2">
                  ...
                  <Button
                    onClick={() =>
                      onPlaceBid(
                        player.id,
                        player.playerMaster.nickname ||
                          player.playerMaster.name,
                    ...
                    disabled={
                      bidLoading ||
                      !bidAmount ||
                      Number.parseInt(bidAmount, 10) <= player.salePrice
                    }
                    size="sm"
                  >
                ...
                {bidSuccess && (
                  <div className="text-xs text-green-600">{bidSuccess}</div>
                )}
