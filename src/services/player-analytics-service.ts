...
export class PlayerAnalyticsService {
    async getPlayerMarketTrend(cookie: string, playerId: string): Promise<ApiResponse<MarketValuePoint[]>> {
        return apiClient.get<MarketValuePoint[]>(endpoints.player.marketValue(playerId), cookie);
    }

    private calculateTrendPercentage(marketValueData: MarketValuePoint[], days: number): number {
    ...
    private calculateMomentumScore(trends: { last1Days: number; last3Days: number; last7Days: number }): number {
        ...
        return parseFloat((momentum + shortTrend + weeklyContext).toFixed(2));
    }

    async enrichPlayerWithAnalysis(cookie: string, player: Player): Promise<Player> {
    ...
    async enrichPlayersWithAnalysis(cookie: string, players: Player[]): Promise<Player[]> {
    ...
    static calculateSummaryStats(players: Player[]) {
        ...
        const averagePoints = players.length > 0 ? parseFloat((totalPoints / players.length).toFixed(1)) : 0;
    ...
    static getTrendingUpPlayers(players: Player[], minimumMomentum: number = 5): Player[] {
        return players.filter(p => p.analysis?.momentumScore && p.analysis.momentumScore > minimumMomentum);
    }

    static getUrgentDeals(players: Player[], hoursThreshold: number = 12): Player[] {
        return players.filter(p => {
            if (!p.saleInfo?.expirationDate) return false;
            const hoursLeft = (new Date(p.saleInfo.expirationDate).getTime() - new Date().getTime()) / (1000 * 60 * 60);
